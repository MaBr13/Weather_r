##########################################
#####Working with bioRad and vol2bird#####
##########################################
...
setwd("C:/test-dataset-judy/2016/10/07/23")#set a directory where your files are
...
pvols <- sapply(listmpvols, read.pvol)#using sapply here as it produces matrix instead of a list which lapply gives,
...
ppis <- lapply(allscans, ppi,range.max=50000, cellsize=500)
#vol2bird automate
...
mypath <- "C:/test-dataset-judy/2016/10/07/17"
vps <- retrieve_vp_paths(mypath, start_date = "2016-10-07",end_date = "2016-10-07",country = c("nl"), radar=c("dbl"))

lvps <- readvp.list(vps)
ts <- vpts(lvps)
regts <- regularize(ts)
plot(regts)
